...
class TestResidualLayer(TestCase):
    ...
    def setUp(self):
        ...
        self.residual_transformation = functional_layers.LambdaLayer(
            activations.sigmoid
        )
        self.layer_weight = 0.444
        self.residual_weight = 0.222
    ...
    def test_residual_transformation(self):
        # Create residual layer
        residual = residual_layers.ResidualLayer(
            self.layer,
            residual_transform=self.residual_transformation,
        ...
        for x_val in self.inputs:
            ...
            z = self.layer_weight * activations.tanh(x) + \
                self.residual_weight * activations.sigmoid(x)
            # Check value
